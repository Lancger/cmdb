...
if len(sys.argv) > 2:
    ...
    if sys.argv[1] == 'runworker':
        if sys.argv[2].startswith('--only-channels='):
            chanel = sys.argv[2].replace('--only-channels=', '')
            port = chanel.split('.')[0]
        ...
        if len(port) == 1:
            ...
            # 0.0.0.0:8088
            port = port[1]
    elif sys.argv[0].endswith('daphne'):
        port = sys.argv[6]

    if not port.isdigit():
        port = '0000'
    web_channels = ['http.request', 'websocket.connect', 'websocket.receive', 'websocket.disconnect']

    for c in web_channels:
        new_channels.update({c: '%s.%s' % (port, c)})
    # print new_channels, '!!!!'

    import asgi_redis
    old_RedisChannelLayer = asgi_redis.RedisChannelLayer


    class new_RedisChannelLayer(old_RedisChannelLayer):

        def send(self, channel, message):
            # print 'channel', channel, 7777777777777777777777777777777
            if channel in web_channels:
                channel = new_channels[channel]
            # print 'channel', channel, 9999999999999999999999999999999
            return super(new_RedisChannelLayer, self).send(channel, message)

    asgi_redis.RedisChannelLayer = new_RedisChannelLayer
#############################################################################
...
class Router(object):
    ...
    def check_default(self, http_consumer=None):
        ...
        self.add_route(Route(new_channels["http.request"], http_consumer or ViewConsumer()))
        ...
        self.add_route(Route(new_channels["websocket.connect"], connect_consumer))
        self.add_route(Route(new_channels["websocket.receive"], null_consumer))
        self.add_route(Route(new_channels["websocket.disconnect"], null_consumer))
