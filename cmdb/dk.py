...
from cmdb.models import DockerHost, docker, DockerYml, DockerCompose, DockerImageFile
...
class DockerHostList(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    template_name = "docker/host_list.html"
    model = DockerHost
    permission_required = 'cmdb.view_dockerhost'
...
def docker_client(request, pk):
    ...
    if not request.user.has_perm('cmdb.change_dockerhost'):
...
def info(request, pk):
    ...
    try:
        ...
        pass
    return render(request, 'docker/info.html', locals())
...
def image(request, pk=0):
    ...
    return render(request, 'docker/image.html', locals())
...
def container(request, pk=0):
    ...
    return render(request, 'docker/container.html', locals())
...
def container_add(request, pk):
        ...
        if form.is_valid():
            ...
            try:
                ...
                if start == 'on':
                    ...
                    container.start()
                success_url = reverse_lazy('cmdb:docker_container', kwargs={'pk': pk})
    ...
    return render(request, 'docker/container_add.html', locals())
...
def net(request, pk=0):
    ...
    return render(request, 'docker/net.html', locals())
...
def net_add(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                success_url = reverse_lazy('cmdb:docker_net', kwargs={'pk': pk})
                return redirect(success_url)
            except Exception as e:
                form.errors.update({'错误': str(e)})
    ...
    # 打开表单页面
    return render(request, 'docker/net_add.html', locals())
...
class DockerWebSSH(LoginRequiredMixin, View):
    ...
    def get(self, request):
        user = request.user
        return render_to_response('docker/webssh.html', locals())


class DockerYmlList(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    template_name = "docker/yml_list.html"
    model = DockerYml
    permission_required = 'cmdb.view_dockeryml'


class DockerYmlView(LoginRequiredMixin, PermissionRequiredMixin):
    template_name = "docker/yml.html"
    model = DockerYml
    form_class = DockerYmlForm
    success_url = reverse_lazy('cmdb:docker_yml_list')
...
class DockerYmlAdd(DockerYmlView, CreateView):
    permission_required = 'cmdb.can_add_dockeryml'


class DockerYmlEdit(DockerYmlView, UpdateView):
    permission_required = 'cmdb.can_edit_dockeryml'
...
class DockerComposeList(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    template_name = "docker/compose_list.html"
    model = DockerCompose
    permission_required = 'cmdb.view_dockercompose'
...
class DockerComposeView(LoginRequiredMixin, PermissionRequiredMixin):
    template_name = "docker/compose.html"
    model = DockerCompose
    form_class = DockerComposeForm
    success_url = reverse_lazy('cmdb:docker_compose_list')
...
class DockerComposeAdd(DockerComposeView, CreateView):
    permission_required = 'cmdb.can_add_dockercompose'


class DockerComposeEdit(DockerComposeView, UpdateView):
    permission_required = 'cmdb.can_edit_dockercompose'
...
def docker_compose_del(request, ids=''):
    # print ids, 8888
    # import ipdb; ipdb.set_trace()
    if not request.user.has_perm('cmdb.delete_dockercompose'):
        raise PermissionDenied
...
class DockerImageFileList(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    template_name = "docker/imagefile_list.html"
    model = DockerImageFile
    permission_required = 'cmdb.view_dockerimagefile'
...
def docker_imagefile_del(request, ids=''):
    # print ids, 8888
    # import ipdb; ipdb.set_trace()
    if not request.user.has_perm('cmdb.delete_dockerimagefile'):
        raise PermissionDenied
    ret = {"remove": "ok", "status": True}
    try:
        # ids = [id.strip() for id in ids.split(',')]
...
class DockerImageFileLoad(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    permission_required = 'cmdb.can_edit_dockerimagefile'
...
class DockerComposeUpload(LoginRequiredMixin, PermissionRequiredMixin, View):
    # 手工上传镜像包到资产系统中
    permission_required = 'cmdb.can_add_dockerimagefile'
